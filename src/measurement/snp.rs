...
impl SnpLaunchDigest {
    ...
    /// Get the launch digest as a byte slice
...
pub fn snp_calc_launch_digest(
    ...
    let mut gctx: Gctx<Updating> = match snp_measurement.ovmf_hash_str {
        Some(hash) => {
            ...
            eprintln!("[SNP_DEBUG] Rust: LD after GCTX init: {}", hex_encode(gctx.ld.as_slice()));
            gctx
        }
        None => {
            let mut gctx = Gctx::default();
            ...
            if let Some(ovmf) = &ovmf {
                ...
                eprintln!("[SNP_DEBUG] Rust:   LD before OVMF data update: {}", hex_encode(gctx.ld.as_slice()));
                gctx.update_page(PageType::Normal, ovmf.gpa(), Some(ovmf.data()), None)?;
                eprintln!("[SNP_DEBUG] Rust:   LD after OVMF data update: {}", hex_encode(gctx.ld.as_slice()));
            }
    ...
    eprintln!("[SNP_DEBUG] Rust: LD after OVMF metadata parsing: {}", hex_encode(gctx.ld.as_slice()));
    ...
    eprintln!("[SNP_DEBUG] Rust: LD before VMSA updates: {}", hex_encode(gctx.ld.as_slice()));
    ...
    for (i, vmsa_page) in vmsa_pages.iter().enumerate() {
        ...
        eprintln!("[SNP_DEBUG] Rust:   LD before this VMSA update: {}", hex_encode(gctx.ld.as_slice()));
        gctx.update_page(PageType::Vmsa, VMSA_GPA, Some(vmsa_page.as_slice()), None)?;
        eprintln!("[SNP_DEBUG] Rust:   LD after this VMSA update:  {}", hex_encode(gctx.ld.as_slice()));
    }
    eprintln!("[SNP_DEBUG] Rust: LD after all VMSA updates: {}", hex_encode(gctx.ld.as_slice()));

    // Finalize Gctx and return the launch digest
    let gctx = gctx.finished();
    eprintln!("[SNP_DEBUG] Rust: Final launch digest: {}", hex_encode(gctx.ld().as_slice()));
    Ok(gctx.ld())
}